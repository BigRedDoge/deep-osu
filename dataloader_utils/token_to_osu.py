...
class TokenToOsu:
    ...
    def _dequantize_coord(self, bin_index, dimension="x"):
        ...
        num_bins = self.quantizers.coord_bins
        if num_bins <= 0: return 0.0

        # Clamp index just in case
        clamped_index = max(0, min(bin_index, num_bins - 1))

        if dimension == "x":
            max_coord = self.quantizers.playfield_width
        elif dimension == "y":
            max_coord = self.quantizers.playfield_height
        else:
            raise ValueError("Dimension must be 'x' or 'y'")

        # Calculate the coordinate corresponding to the center of the bin
        coord = (clamped_index + 0.5) * (max_coord / num_bins)
        # Clamp coordinate to valid osu! range
        return max(0.0, min(coord, max_coord))


    def _dequantize_spinner_duration(self, bin_index):
        """Converts an END_SPINNER_DUR bin index back to milliseconds (linear)."""
        num_bins = self.quantizers.spinner_duration_bins
        if num_bins <= 1: return 0.0
        # Clamp index just in case
        clamped_index = max(0, min(bin_index, num_bins - 1))
        # Center of the bin's duration range
        return (clamped_index + 0.5) * (self.quantizers.max_spinner_duration_ms / num_bins)
        ...
        for token_id in token_ids:
            ...
            if token_str.startswith("TIME_SHIFT_"):
                try:
                    ...
                    dt = self._dequantize_time(bin_index)
                    current_time += dt
                ...
                try:
                    bin_index = int(token_str.split('_')[-1])
                    last_x = self._dequantize_coord(bin_index, "x")
                except (ValueError, IndexError):
                ...
                try:
                    bin_index = int(token_str.split('_')[-1])
                    last_y = self._dequantize_coord(bin_index, "y")
                except (ValueError, IndexError):
                ...
                hit_objects_str.append(
                    ...
                    # f"{pixelLength}" # Omitted pixelLength
                )
                ...
                try:
                    ...
                    duration = self._dequantize_spinner_duration(bin_index)
