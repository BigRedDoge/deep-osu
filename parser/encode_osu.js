...
function getPathTypeEnum(char) { 
    switch (char) {
        ...
        default: console.warn(`Warning: Unknown slider path type char '${char}'. Defaulting to Bezier.`); return PathType.Bezier; }
}
...
async function encodeBeatmapFromInput() {
    ...
    try {
        ...
        beatmap.general = new BeatmapGeneralSection(); 
        beatmap.general.mode = 0; 
        beatmap.general.sampleSet = SampleSet.Normal; 
        beatmap.general.stackLeniency = 0.7;
        beatmap.general.audioFilename = meta.audioFilename || "audio.mp3"
        
        beatmap.metadata = new BeatmapMetadataSection(); 
        beatmap.metadata.title = meta.title || "AI Generated Title"; 
        beatmap.metadata.titleUnicode = meta.titleUnicode || beatmap.metadata.title; 
        beatmap.metadata.artist = meta.artist || "AI Artist"; 
        beatmap.metadata.artistUnicode = meta.artistUnicode || beatmap.metadata.artist;
        beatmap.metadata.creator = meta.creator || "TokenToOsu"; 
        beatmap.metadata.version = meta.version || "Generated"; 
        beatmap.metadata.source = meta.source || ""; 
        beatmap.metadata.tags = Array.isArray(meta.tags) ? meta.tags : (meta.tags ? String(meta.tags).split(' ') : []); 
        beatmap.metadata.beatmapId = meta.beatmapId || -1; 
        beatmap.metadata.beatmapSetId = meta.beatmapSetId || -1;

        beatmap.difficulty = new BeatmapDifficultySection(); 
        const diff = inputData.difficulty || {}; 
        beatmap.difficulty.drainRate = diff.HPDrainRate ?? 5; 
        beatmap.difficulty.circleSize = diff.CircleSize ?? 5; 
        beatmap.difficulty.overallDifficulty = diff.OverallDifficulty ?? 5; 
        beatmap.difficulty.approachRate = diff.ApproachRate ?? beatmap.difficulty.overallDifficulty; 
        beatmap.difficulty.sliderMultiplier = diff.SliderMultiplier ?? 1.4; 
        beatmap.difficulty.sliderTickRate = diff.SliderTickRate ?? 1;
        ...
        if (receivedHitObjects && Array.isArray(receivedHitObjects)) {
            ...
            receivedHitObjects.forEach((objData, index) => {
                ...
                const startX = objData.x ?? 256; 
                const startY = objData.y ?? 192; 
                const startPos = new Vector2(startX, startY);
                const isNewCombo = objData.is_new_combo ?? false; 
                const hitSound = HitSound.Normal; 
                let currentHitType = 0;
                if (isNewCombo) { 
                ...
                try {
                    ...
                    switch (objData.object_type) {
                        ...
                        case 'Circle':
                            ...
                            hitObject.isNewCombo = isNewCombo; 
                            console.warn("DEBUG: HittableObject created."); 
                            break;
                        case 'Slider':
                            console.warn("DEBUG: Creating SlidableObject..."); 
                            currentHitType |= HitType.Slider; 
                            const slider = new SlidableObject(); 
                            slider.isNewCombo = isNewCombo; 
                            ...
                            if (objData.points && Array.isArray(objData.points) && objData.points.length >= 2) {
                                ...
                                const controlPoints = []; 
                                const newPath = new PathPoint(new Vector2(0, 0), pathType);
                                controlPoints.push(newPath); 
                                ...
                                for (let i = 1; i < objData.points.length; ++i) { 
                                    ...
                                    const pointX = p?.x ?? startX; 
                                    const pointY = p?.y ?? startY; 
                                ...
                                slider.path.controlPoints.push(new PathPoint(new Vector2(0, 0), PathType.Bezier));
                                slider.path.controlPoints.push(new PathPoint(new Vector2(1, 0))); 
                            }
                            hitObject = slider; 
                            hitObject.path.curveType = getPathTypeEnum(objData.slider_type_char);
                            console.warn("DEBUG: SlidableObject created."); 
                            break;
                        case 'Spinner': 
                            console.warn("DEBUG: Creating SpinnableObject..."); 
                            currentHitType |= HitType.Spinner; 
                            const spinner = new SpinnableObject(); 
                            spinner.isNewCombo = isNewCombo; 
                            spinner.endTime = objData.end_time ?? (startTime + 1000); 
                            hitObject = spinner; 
                            console.warn(`DEBUG: SpinnableObject created, end time: ${spinner.endTime}`); break;
                        default:
                            console.warn(`Warning: Unknown hit object type '${objData.object_type}'. Skipping.`); return;
                    }
                    ...
                    */
                    ...
                    normalSample.volume = sp?.volume ?? 0;
                    hitObject.samples.unshift(normalSample);
